...
class TestCapDelegateTaskCalls:
    ...
    def test_at_limit_passes_through(self):
        max_children = _get_max_concurrent_children()
        tcs = [make_tc("delegate_task") for _ in range(max_children)]
        out = AIAgent._cap_delegate_task_calls(tcs)
        assert out is tcs

    def test_below_limit_passes_through(self):
        max_children = _get_max_concurrent_children()
        tcs = [make_tc("delegate_task") for _ in range(max_children - 1)]
        out = AIAgent._cap_delegate_task_calls(tcs)
        assert out is tcs
    ...
    def test_interleaved_order_preserved(self):
        ...
        out = AIAgent._cap_delegate_task_calls(tcs)
        expected = [delegates[0], t1, delegates[1], w1] + delegates[2:max_children]
