...
Responsibilities (Task #7 scope):
  - Detect entry phrases and explicit exit commands
  - Create / remove the per-session state file
...
_SAFE_STATE_COMPONENT_RE = re.compile(r"[^A-Za-z0-9._=-]+")
_SAFE_STATE_COMPONENT_MAX = 96
...
def _agent_value(parent_agent: Any, public_name: str, private_name: str) -> str | None:
    """兼容 AIAgent 的公开属性和内部字段。"""
...
def _safe_state_component(value: str | None) -> str:
    raw = str(value or "").strip()
    if not raw:
        return "default"
    safe = _SAFE_STATE_COMPONENT_RE.sub("_", raw).strip("._-")
    if not safe:
        safe = "session"
    if safe == raw and len(safe) <= _SAFE_STATE_COMPONENT_MAX:
        return safe
    digest = hashlib.sha256(raw.encode("utf-8")).hexdigest()[:12]
    safe_max = _SAFE_STATE_COMPONENT_MAX - len(digest) - 1
    safe = safe[:safe_max].rstrip("._-") or "session"
    return f"{safe}-{digest}"


def _session_state_identity(parent_agent: Any) -> tuple[str, dict[str, Any]]:
    """返回深度养马模式的会话级状态身份。"""
    gateway_session_key = _agent_value(
        parent_agent, "gateway_session_key", "_gateway_session_key"
    )
    user_id = _agent_value(parent_agent, "user_id", "_user_id")
    chat_id = _agent_value(parent_agent, "chat_id", "_chat_id")
    thread_id = _agent_value(parent_agent, "thread_id", "_thread_id")
    platform = _agent_value(parent_agent, "platform", "_platform")
    session_id = _agent_value(parent_agent, "session_id", "_session_id")

    if gateway_session_key:
        identity_source = "gateway_session_key"
        identity = _safe_state_component(gateway_session_key)
    elif any((chat_id, thread_id, user_id)):
        identity_source = "platform_chat_thread_user"
        identity = "_".join(
            _safe_state_component(part)
            for part in (
                platform or "unknown",
                chat_id or "nochat",
                thread_id or "nothread",
                user_id or "nouser",
            )
        )
    elif session_id:
        identity_source = "session_id"
        identity = "session_" + _safe_state_component(session_id)
    elif platform:
        identity_source = "platform"
        identity = _safe_state_component(platform)
    else:
    ...
    return identity, {
        ...
        "state_identity_source": identity_source,
...
def _state_file_path(
    project_root: Path, state_identity: str | None, channel_id: str | None = None
) -> Path:
    if channel_id is not None:
        state_identity = f"{state_identity or 'default'}_{channel_id or 'default'}"
    filename = f"skill_creation_{_safe_state_component(state_identity)}.json"
    return _state_dir(project_root) / filename


def _state_file_path_for_agent(project_root: Path, parent_agent: Any) -> Path:
    state_identity, _ = _session_state_identity(parent_agent)
    return _state_file_path(project_root, state_identity)
...
def _strip_feishu_reply_context(message: str) -> str:
    """移除飞书注入的回复引用，只保留当前消息正文。"""
    m = _FEISHU_REPLY_RE.match(message)
    if not m:
        return message
    return message[m.end() :].lstrip()


def _normalize_command_message(message: str) -> str:
    current = _strip_feishu_reply_context(message).strip()
    current = _LEADING_SPEAKER_RE.sub("", current, count=1).strip()
    return current.strip(_COMMAND_TRIM_CHARS).casefold()


def _match_command_phrase(message: str, phrases: tuple[str, ...]) -> bool:
    if not phrases:
        return False
    normalized = _normalize_command_message(message)
    return any(normalized == p.casefold() for p in phrases)
...
def _enter_mode(
    ...
    *,
    ...
    initial_state = {
        ...
        "state_identity_source": state_scope.get("state_identity_source"),
    ...
    try:
        from agent_system.experts.builder_expert.builder import render_initial_reply

        response_text = render_initial_reply(initial_state, parent_agent)
    except Exception as exc:
        logger.warning("builder initial reply rendering failed, using fallback: %s", exc)
        response_text = (
            "🐎 **深度养马模式已启动**\n\n"
    ...
    return _final_response(response_text)
...
def maybe_handle_builder_mode(
    ...
    *,
    ...
    state_identity, state_scope = _session_state_identity(parent_agent)
    user_id = state_scope.get("user_id") or "default"
    channel_id = state_scope.get("channel_id") or "default"
    state_path = _state_file_path(project_root, state_identity)
    current_message = _strip_feishu_reply_context(user_message)
    if not current_message.strip():
        return None

    # Exit takes precedence even when not in mode (idempotent).
    if _match_command_phrase(current_message, _EXIT_PHRASES):
        return _exit_mode(state_path)

    if _match_phrase(current_message, _ENTRY_PHRASES):
    ...
    if state_path.exists():
        return _dispatch_to_state_machine(state_path, current_message, parent_agent)
