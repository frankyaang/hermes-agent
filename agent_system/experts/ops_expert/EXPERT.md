...
| 专家 ID | ops_expert |
| 专家名称 | 运营专家 |
| 业务范围 | 负责产品运营报告、Dashboard结构和运营动作建议 |
| 角色定位 | 主专家 / 主智能体 |
...
| --- | --- | --- | --- | --- | --- |
| `ops_dashboard` | 运营看板 | 90 | 生成 / 输出整理 | 高权重默认加载 | active |
...
| `node_id` | 文本 | 必填 | 无 | 当前任务节点 | 来自 routes | ops_dashboard |
| `expert_id` | 文本 | 必填 | 无 | 主专家 ID | 来自 experts | ops_expert |
...
| `default_skill_weights` | JSON | 可选 | 分值 | 0-100 | Skill ID 动态扩展 | `{ops_dashboard: 90}` |
...
| Skill 使用 | `skills_loaded` | 列表 | 个 | 至少 1 个 | `ops_dashboard` |
| Skill 权重 | `skill_weights` | JSON | 分值 | 0-100 | `{ops_dashboard: 90}` |
| 主专家执行 | `primary_expert_skill_calls` | 列表 | 次 | 可为空 | `ops_dashboard` |
...
| `task_count` | 数字 | 个 | 12 |
| `primary_skill_usage` | JSON | 次 | `{ops_dashboard: 10}` |
...
| `weight_adjustment_suggestions` | JSON | 分值变化 | `{ops_dashboard: +5}` |
...
| 模板占位 | 扫描 `ops_expert`、`ops_dashboard`、`human_review_required`、`human_input_summary` 等字段 | 占位完整，可被生成器替换 |
