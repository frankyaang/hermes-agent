...
| 运行闭环落地 | 执行 DAG、主/辅专家平行调用 Skill、写审计日志、生成复盘摘要、回写权重、沉淀三层私域经验 | 运行产物必须包含 `audit.jsonl`、`review_summary.json`、`skill_weights.json`、系统级经验、专家级经验和 Skill 级经验摘要 |
...
| 三层经验隔离 | 系统级、专家级、Skill 级经验独立写入，不跨层覆盖 | `review_summary.json` 记录 `experience_isolation` 和 `experience_updates` |
...
| 专家 ID | render_expert |
| 专家名称 | 渲染专家 |
| 业务范围 | 负责HTML/PDF呈现质量，确保渲染层不改写业务内容 |
| 角色定位 | 主专家 / 主智能体 |
...
| 层级 | 维护主体 | 内容范围 | 写入规则 | 运行产物 |
| --- | --- | --- | --- | --- |
| 系统级经验 | Hermes 主代理 / 全局运行器 | 全局策略、用户偏好、会话记忆、运行级复盘索引 | 只由 Hermes 主代理或全局逻辑写入；临时子专家不直接写入 | `memory/system_mem/MEMORY.md` |
| 专家级经验 | 对应专家 | 专家决策、历史任务、复盘结果、Skill 调用历史 | 只写入本专家 `expert_mem`；不覆盖 Skill 经验 | `experts/{expert_id}/expert_mem/MEMORY.md` |
| Skill 级经验 | 对应 Skill | Skill 执行历史、成功率、异常统计、复盘反馈 | 只写入本 Skill `skill_mem`；不覆盖专家经验 | `skills/{skill_id}/skill_mem/MEMORY.md` |

### 3.2 访问与写入规则

| 执行主体 | 可读经验 | 可写经验 | 边界说明 |
| --- | --- | --- | --- |
| Hermes 主代理 | 系统级、专家级摘要、Skill 级摘要 | 系统级经验 | 负责全局偏好、系统策略和边界不清时的归属判定 |
| 主专家 / 辅专家 | 系统级经验、本专家经验、相关 Skill 摘要 | 本专家级经验 | 记录任务判断、节点结果、复盘和 Skill 使用摘要 |
| Skill | 系统级经验、对应专家经验、本 Skill 经验 | 本 Skill 级经验 | 记录执行历史、异常、输出质量和复盘反馈 |
| 临时子专家 | 任务相关系统级摘要、对应专家经验、相关 Skill 经验 | 对应专家级或 Skill 级经验 | 不直接写系统级经验；由 Hermes 主代理在边界不清时判定归属 |

### 3.3 运行字段

| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `memory_access_policy` | JSON | 当前节点可读 / 可写的系统级、专家级、Skill 级经验边界 |
| `experience_write_scope` | JSON | 当前节点允许写入的层级和目标经验库 |
| `experience_isolation` | JSON | 复盘摘要中的三层隔离规则说明 |
| `experience_updates` | 列表 | 本次运行实际写入的经验层级、owner、路径和原因 |

### 3.4 隔离原则

- 三层经验各自独立管理，不存在跨层覆盖逻辑。
- 专家级经验可以记录 Skill 使用摘要，但不能替代或改写 Skill 级经验。
- Skill 级经验可以记录执行反馈，但不能写入专家决策或系统级偏好。
- 系统级经验只保存全局策略、用户偏好、会话记忆和运行级复盘索引。
- 复盘摘要只做索引和路径记录，不把三层经验正文混写在一起。

## 4. 默认 Skill 多重加载

| Skill ID | Skill 名称 | Base Weight | 默认用途 | 加载策略 | 当前状态 |
| --- | --- | --- | --- | --- | --- |
| `dashboard_html` | 看板渲染 | 90 | 渲染 / 交付呈现 | 高权重默认加载 | active |
| `briefing` | 过程汇报 | 80 | 过程汇报 / 仿人汇报 | 仿人汇报触发加载 | active |
...
| `node_id` | 文本 | 必填 | 无 | 当前任务节点 | 来自 routes | dashboard_html |
| `expert_id` | 文本 | 必填 | 无 | 主专家 ID | 来自 experts | render_expert |
...
| `default_skill_weights` | JSON | 可选 | 分值 | 0-100 | Skill ID 动态扩展 | `{dashboard_html: 90}` |
...
| Skill 使用 | `skills_loaded` | 列表 | 个 | 至少 1 个 | `dashboard_html` |
| Skill 权重 | `skill_weights` | JSON | 分值 | 0-100 | `{dashboard_html: 90}` |
| 主专家执行 | `primary_expert_skill_calls` | 列表 | 次 | 可为空 | `dashboard_html` |
...
| 经验访问策略 | `memory_access_policy` | JSON | 无 | 明确系统级、专家级、Skill 级可读 / 可写边界 | `{system_level:{read:true}}` |
| 经验写入范围 | `experience_write_scope` | JSON | 无 | 只允许写入自身层级 | `{skill_level:[voc_insight]}` |
| 经验更新清单 | `experience_updates` | 列表 | 条 | 复盘后记录层级、owner、路径和原因 | `[{layer:skill, owner:voc_insight}]` |
...
| 三层经验隔离 | 系统级、专家级、Skill 级分别写入不同路径 | `experience_updates` |
| 经验写入边界 | 写入主体只能写自身层级，不跨层覆盖 | `memory_access_policy`、`experience_write_scope` |
...
| `task_count` | 数字 | 个 | 12 |
| `primary_skill_usage` | JSON | 次 | `{dashboard_html: 10}` |
...
| `weight_adjustment_suggestions` | JSON | 分值变化 | `{dashboard_html: +5}` |
| `rule_update_suggestions` | 列表 | 条 | 新增人工复核触发条件 |
| `experience_isolation` | JSON | 无 | 三层经验隔离规则 |
| `experience_updates` | 列表 | 条 | 系统级 / 专家级 / Skill 级写入记录 |
...
| `memory_access_policy` | JSON | 三层经验访问与写入规则 |
| `experience_write_scope` | JSON | 当前节点允许写入的经验层级 |
| `experience_updates` | 列表 | 本次复盘沉淀到三层经验库的索引 |
...
| 层级 | 写入内容 | 禁止事项 | 复盘用途 |
| --- | --- | --- | --- |
| 系统级经验 | 运行级全局摘要、用户偏好、全局策略信号 | 不写 Skill 内部执行正文，不写专家私有判断原文 | 识别全局偏好和系统策略变化 |
| 专家级经验 | 专家决策、节点参与、异常复核、Skill 调用摘要 | 不覆盖 Skill 成功率和执行历史 | 优化专家分工和后续 Skill 调用选择 |
| Skill 级经验 | Skill 调用次数、完成情况、输出质量、异常统计 | 不写专家决策或系统级用户偏好 | 优化 Skill 权重、动态覆盖和异常处理 |

运行器必须把本次经验沉淀写入 `review_summary.json` 的 `experience_updates`，并追加到 `audit/experience_updates.jsonl`，用于后续复盘追踪。
...
| 模板占位 | 扫描 `render_expert`、`dashboard_html`、`human_review_required`、`human_input_summary` 等字段 | 占位完整，可被生成器替换 |
...
| 三层经验字段 | 扫描系统级、专家级、Skill 级、`memory_access_policy`、`experience_updates` | 字段能证明三层经验隔离，不混写 |
...
| 三层私域经验 | 查看系统级、专家级和 Skill 级记忆文件 | 运行摘要按层写入各自 `MEMORY.md`，无跨层覆盖 |
| 经验更新索引 | 查看复盘摘要和 `audit/experience_updates.jsonl` | `experience_updates` 记录层级、owner、路径和写入原因 |
...
| vX.X | YYYY-MM-DD | Owner | 新增系统级 / 专家级 / Skill 级三层经验隔离与写入边界 |
