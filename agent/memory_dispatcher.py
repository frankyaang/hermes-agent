...
def dispatch_event(
    ...
    *,
    ...
    write_to_store=False 时只做逻辑断言，不写文件（测试用）。
    """
    ...
    if event.source_type == "user_correction" and (event.project_hint or event.product_line_hint):
        ...
        if write_to_store:
            scope_target = event.project_hint or event.product_line_hint
            rel_id = write_relation(
                relation_type="user_correction",
                source_id=event.id,
            ...
            vs_id = write_relation(
                ...
                source_id=event.id,
                target_id=scope_target,
                note=f"用户纠正覆盖旧版本内容，scope={scope_target}",
                hermes_home=hermes_home,
            )
...
def dispatch_tool_failure(
    ...
    """knowledge_write permission_denied 后的便捷入口，生成 tool_failure MemoryEvent。"""
