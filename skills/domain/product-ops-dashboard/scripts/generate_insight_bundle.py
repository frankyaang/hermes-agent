...
def render_series_report(result: dict[str, object], *, analysis_title: str, time_scope: str, market: str | None, analysis_goal: list[str]) -> str:
    scaffold_args = SimpleNamespace(
        ...
        period_id=[],
        market=market,
        force_full_goal_codes=True,
    )
    return build_scaffold(scaffold_args, result)
...
def render_portfolio_full_report(
    *,
    ...
    level_rank = {"不支持": 0, "部分支持": 1, "支持": 2}
    combined_support_map: dict[str, dict[str, object]] = {}
    for payload in series_payloads:
        for row in payload["result"]["support_matrix"]:  # type: ignore[index]
            code = row["module_code"]
            candidate = dict(row)
            candidate["evidence"] = f"{payload['series_label']}:{row['evidence']}"
            existing = combined_support_map.get(code)
            if not existing or level_rank[str(candidate["support_level"])] > level_rank[str(existing["support_level"])]:
                combined_support_map[code] = candidate
            elif existing:
                existing["evidence"] = f"{existing['evidence']}；{payload['series_label']}:{row['evidence']}"

    goal_codes = infer_goal_codes(analysis_goal_text)
    feature_exposure_rows = build_feature_exposure_rows(
        goal_codes,
        combined_support_map,
        has_direct_survey=any(payload["result"]["survey_rows"] for payload in series_payloads),  # type: ignore[index]
        has_direct_summary=any(payload["result"]["summary_rows"] for payload in series_payloads),  # type: ignore[index]
        has_direct_raw=any(payload["result"]["raw_rows"] for payload in series_payloads),  # type: ignore[index]
        has_direct_voc=any(payload["result"]["voc_rows"] for payload in series_payloads),  # type: ignore[index]
        has_proxy_survey=False,
        has_proxy_summary=False,
    )
    coverage_gap_rows = build_coverage_gap_rows(feature_exposure_rows)
    goal_feature_rows = build_goal_feature_map_rows(goal_codes)
    action_rows = build_action_plan_rows(
        [],
        [],
        list(combined_support_map.values()),
        load_action_context(),
    )
    series_conclusion_rows = [
        [
            payload["series_label"],
            decision_readiness_label(payload["result"]),
            "、".join(extract_series_scope(payload["result"])["self_spus"]) or "-",
            "当前优先关注高风险本品对象",
        ]
        for payload in series_payloads
    ]
    lines: list[str] = []
    lines.append("# 清洁机器人 X+T 全量本品与竞品用户分析总报告")
    lines.append("")
    lines.append("## 0. 问题回答")
    lines.append("")
    ...
    lines.append(
        markdown_table(
            ["字段", "当前内容"],
            [
                ["分析范围", f"{category_name} 当前已录入的 X + T 全量本品与竞品 cohort"],
                ["分析周期", time_scope],
                ["市场范围", market or "ALL"],
                ["系列覆盖", "、".join(payload["series_label"] for payload in series_payloads)],
                ["数据源", "VOC / 统一问卷 / 访谈总结 / 原始访谈"],
            ],
        )
    )
    lines.append("")
    ...
    lines.append(
        markdown_table(
            ["一级结论", "当前判断", "本品对象", "当前动作焦点"],
            series_conclusion_rows if series_conclusion_rows else [["待补充", "-", "-", "-"]],
        )
    )
    lines.append("")
    lines.append("## 3. 递归论证")
    lines.append("")
    lines.append("### 3.1 系列总览表")
    lines.append("")
    lines.append(
        markdown_table(
            ["系列", "本品对象", "竞品池规模", "VOC覆盖对象", "问卷覆盖波次", "总结覆盖分组", "当前决策可用等级"],
            portfolio_series_overview_rows(series_payloads),
        )
    )
    lines.append("")
    lines.append("### 3.2 全量对象地图")
    lines.append("")
    lines.append(
        markdown_table(
            ["系列", "对象名", "spu_id", "类型", "代际状态"],
            portfolio_object_map_rows(series_payloads),
        )
    )
    lines.append("")

    for idx, payload in enumerate(series_payloads, start=3):
        series_markdown = payload["series_markdown"]
        sections = split_scaffold_sections(series_markdown)
        lines.append(f"### 3.{idx} {payload['series_label']}")
        lines.append("")
        lines.append(f"- 本系列当前决策可用等级：`{decision_readiness_label(payload['result'])}`。")
        lines.append("")
        for title in ("3.1 VOC 单源证据与论证", "3.2 调研问卷证据与论证", "3.3 定性访谈证据与论证", "3.4 智能性拆维论证", "3.5 跨源综合证据与论证"):
            block = sections.get(title)
            if block:
                lines.append(demote_headings(block, 1))
                lines.append("")
    ...
    for payload in series_payloads:
        if not payload["result"]["survey_rows"] or not payload["result"]["summary_rows"]:  # type: ignore[index]
            lines.append(f"- `{payload['series_label']}` 当前仍有模块依赖代理证据或缺少直连研究输入。")
    if coverage_gap_rows:
        lines.append("- 当前仍存在若干特征覆盖缺口，详见上文缺口表。")
    lines.append("")
    ...
    lines.append(
        markdown_table(
            ...
            action_rows if action_rows else [["待补充", "-", "-", "-", "-", "-", "-"]],
        )
    )
    lines.append("")
    ...
    lines.append(
        markdown_table(
            ["系列", "模块", "支持度", "主来源", "证据"],
            [
                [payload["series_label"], row["module_name_cn"], row["support_level"], row["primary_source"], row["evidence"]]
                for payload in series_payloads
                for row in payload["result"]["support_matrix"]  # type: ignore[index]
            ],
        )
    ...
    lines.append("")
    lines.append("### 6.1 当前可以下结论的事")
    lines.append("")
    for line in portfolio_cross_series_priority_lines(series_payloads):
        lines.append(line)
    lines.append("")
    lines.append("### 6.2 当前附录索引")
    lines.append("")
    lines.append("- `x_series_report.md` / `t_series_report.md`：分系列原始证据报告。")
    lines.append("- `before_after_comparison.md`：优化前后对比。")
    lines.append("")
    return "\n".join(lines)
...
def render_main_report(args: argparse.Namespace, result: dict[str, object]) -> str:
    scaffold_args = SimpleNamespace(
...
def write_manifest(
    ...
    *,
    ...
    manifest = {
        "asset_id": "product-ops-dashboard",
...
def main() -> None:
    ...
    if args.bundle_mode == "portfolio_full":
        ...
        main_report = render_main_report(args, result)
        main_report_path = out_dir / "main_report.md"
        presentation_main_report_path = out_dir / "presentation_main_report.md"
        presentation_brief_path = out_dir / "presentation_brief.md"
        summary_insight_cards_path = out_dir / "summary_insight_cards.json"
